...
import { Plus, User, DollarSign, Percent } from 'lucide-react';
...
export const EmployeesView: React.FC<EmployeesViewProps> = ({ employees, onAddEmployee, onDeleteEmployee }) => {
  const [showForm, setShowForm] = useState(false);
  const [newEmp, setNewEmp] = useState({ name: '', role: '', fixedSalary: '', commissionRate: '' });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setNewEmp({ name: '', role: '', fixedSalary: '', commissionRate: '' });
    setShowForm(false);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button 
          ...
          className="bg-moto-600 hover:bg-moto-700 text-white px-4 py-2 rounded-lg flex items-center gap-2 transition-colors"
        >
          <Plus size={18} />
          Novo Funcionário
        </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {employees.map(emp => (
          <div key={emp.id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow">
